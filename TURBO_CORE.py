...
TRANSLATIONS = {
    "PT": {
        "app_title": "TURBO CORE V111 - LINUX EDITION",
...
class TurboCoreApp(ctk.CTk):
    def __init__(self):
        ...
        # Caminhos Externos
        self.exe_dir = os.getcwd() # Usar diretorio atual no Linux
        self.caps_dir = os.path.join(self.exe_dir, "Capturas")

        # Binarios do Sistema (Linux)
        self.adb_exe = "adb"
        ...
        try:
            self.app_generic_icon = ctk.CTkImage(Image.open(self.icon_png_path), size=(20, 20))
        except Exception as e:
        ...
        # Linux Window Icon
        try:
            img = PhotoImage(file=self.icon_png_path)
            self.iconphoto(False, img)
        except Exception as e:
            self.debug_log(f"Erro ao carregar ícone da janela: {e}")
    ...
    def check_binaries(self):
        self.debug_log("Verificando binários no PATH...")
        if not shutil.which("adb"):
             messagebox.showwarning("ERRO CRITICO", "ADB não encontrado no sistema! Instale com: sudo apt install adb")
        if not shutil.which("scrcpy"):
             messagebox.showwarning("ERRO CRITICO", "Scrcpy não encontrado! Instale com: sudo apt install scrcpy")
    ...
    def setup_hotkeys(self):
        if keyboard:
            ...
            except Exception as e: self.debug_log(f"Key error (Root required?): {e}")
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.sidebar, text="V111 LINUX", font=("Roboto", 10), text_color=COLOR_TEXT_DIM).pack(pady=(0, 20))
    ...
    def refresh_apps_list(self):
        ...
        def load():
            ...
            with self.adb_lock:
                res = subprocess.run([self.adb_exe, "-s", self.target_device, "shell", "pm", "list", "packages", "-3"], capture_output=True, text=True)
    ...
    def run_adb_cmd_string(self, cmd_string, show_success=True):
        ...
        def t():
            if not self._ping_device():
                 ...
                 with self.adb_lock:
                     if self.last_ip:
                         subprocess.run([self.adb_exe, "connect", self.last_ip])
                         time.sleep(2)
    ...
    def iniciar_pc(self, choice):
        ...
        def thread_pc():
            ...
            try:
                ...
                with self.adb_lock:
                    subprocess.run([self.adb_exe, "-s", self.target_device, "shell", cmd_size])
                time.sleep(1.5)
                ...
                with self.adb_lock:
                    subprocess.run([self.adb_exe, "-s", self.target_device, "shell", cmd_density])
                time.sleep(0.5)

                # Batch 3: Rotation
                cmd_rot = "settings put system user_rotation 1 && settings put system accelerometer_rotation 0"
                with self.adb_lock:
                    subprocess.run([self.adb_exe, "-s", self.target_device, "shell", cmd_rot])
                time.sleep(0.5)
            ...
            try:
                subprocess.run([self.scrcpy_exe, "-s", self.target_device] + scrcpy_args)
            except Exception as e:
            ...
            try:
                ...
                with self.adb_lock:
                    subprocess.run([self.adb_exe, "-s", self.target_device, "shell", cmds_reset])
            except: pass
    ...
    def _ping_device(self):
        try:
            with self.adb_lock:
                res = subprocess.run([self.adb_exe, "-s", self.target_device, "get-state"], capture_output=True, text=True)
    ...
    def heal_adb_connection(self):
        ...
        with self.adb_lock:
            subprocess.run([self.adb_exe, "disconnect"])
        time.sleep(1)
    ...
    def restaurar_padrao(self):
        ...
        self.run_adb_cmd_string(cmds, show_success=False)
    ...
    def install_apk(self):
        ...
        def run():
            with self.adb_lock:
                res = subprocess.run([self.adb_exe, "-s", self.target_device, "install", "-r", file_path], capture_output=True, text=True)
    ...
    def send_file(self):
        ...
        def run():
            with self.adb_lock:
                res = subprocess.run([self.adb_exe, "-s", self.target_device, "push", file_path, "/sdcard/Download/"], capture_output=True, text=True)
            if res.returncode == 0:
                self.log("File Sent!")
        ...
        def run():
            try:
                with open(filepath, "wb") as f:
                    with self.adb_lock:
                        subprocess.run([self.adb_exe, "-s", self.target_device, "exec-out", "screencap", "-p"], stdout=f)
    ...
    def start_logcat(self):
        ...
        def run():
            try:
                self.logcat_process = subprocess.Popen([self.adb_exe, "-s", self.target_device, "logcat", "-v", "time"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def get_device_name(self):
        try:
            with self.adb_lock:
                res = subprocess.run([self.adb_exe, "-s", self.target_device, "shell", "getprop ro.product.model"], capture_output=True, text=True, timeout=2)
    ...
    def start_monitor(self):
        def loop():
            while True:
                ...
                try:
                    # No Linux, apenas testamos se adb esta no path e rodamos
                    with self.adb_lock:
                        res = subprocess.run([self.adb_exe, "devices"], capture_output=True, text=True)
            ...
            while True:
                if self.target_device:
                    try:
                        # Battery
                        with self.adb_lock:
                            res = subprocess.run([self.adb_exe, "-s", self.target_device, "shell", "dumpsys", "battery"], capture_output=True, text=True)
                        level = re.search(r'level: (\d+)', res.stdout)
                        temp = re.search(r'temperature: (\d+)', res.stdout)

                        # Storage (df -h /data)
                        with self.adb_lock:
                            res_st = subprocess.run([self.adb_exe, "-s", self.target_device, "shell", "df", "-h", "/data"], capture_output=True, text=True)
                        avail = "N/A"
                        if res_st.stdout:
                        ...
                        # RAM (/proc/meminfo)
                        with self.adb_lock:
                            res_mem = subprocess.run([self.adb_exe, "-s", self.target_device, "shell", "cat", "/proc/meminfo"], capture_output=True, text=True)
                        ram_str = "RAM: N/A"
                        if res_mem.stdout:
    ...
    def run_manual(self):
        ...
        def run_thread():
            ...
            # Remove bin path substitution logic as they are in PATH now.
            # We can optionally quote them if the user typed "adb" without quotes and it's being run in shell=True
            # But usually 'adb' works fine.

            try:
                # 3. Execute
                with self.adb_lock:
                    # Removed cwd=self.bin_dir
                    res = subprocess.run(cmd_final, shell=True, capture_output=True, text=True)
    ...
    def run_adb_generic(self, cmd, show_success=False):
        def t():
            with self.adb_lock:
                subprocess.run([self.adb_exe] + cmd.split())
            if show_success:
                self.after(0, lambda: messagebox.showinfo(self.T("msg_success"), self.T("msg_cmd_success")))
        threading.Thread(target=t).start()
