            ...
            Box(modifier = Modifier.fillMaxSize().background(Color(0xFF0A0A0A))) {
                LazyColumn(modifier = Modifier.padding(16.dp)) {
                    item {
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) {
                                terminalLog = AppManager.runCommand("wm size 540x960 && wm density 180")
                            }
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) {
                                terminalLog = AppManager.runCommand("wm size reset && wm density reset && settings put global low_power 0 && pm unsuspend com.google.android.gms && cmd power set-fixed-performance-mode-enabled false")
                            }
