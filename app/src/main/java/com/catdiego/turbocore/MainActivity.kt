            ...
            Box(modifier = Modifier.fillMaxSize().background(Color(0xFF0A0A0A))) {
                LazyColumn(modifier = Modifier.padding(16.dp)) {
                    item {
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("wm size 540x1200 && wm density 210") }
                        }, modifier = Modifier.fillMaxWidth()) { Text("MODO BRUTO (720p)") }

                        Spacer(Modifier.height(8.dp))

                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("wm size reset && wm density reset") }
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("settings put global low_power 1 && pm suspend com.google.android.gms") }
                        }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF4CAF50))) { Text("SUPER ECONOMIA") }

                        Spacer(Modifier.height(8.dp))

                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("wm size 360x800 && settings put global low_power 1") }
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("cmd power set-fixed-performance-mode-enabled true") }
                        }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2196F3))) { Text("GAMER ULTIMATE") }

                        Spacer(Modifier.height(8.dp))

                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("wm density 180") }
                        ...
                        Button(onClick = {
                            scope.launch(Dispatchers.IO) { terminalLog = AppManager.runCommand("wm size reset && wm density reset && settings put global low_power 0") }
