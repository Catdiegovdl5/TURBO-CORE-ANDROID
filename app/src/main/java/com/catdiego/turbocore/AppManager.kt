...
object AppManager {
    suspend fun runCommand(context: Context, command: String): String = withContext(Dispatchers.IO) {
        ...
        // V205: Thermal Guard Ativo
        if (ThermalWatchdog.isOverheating(context)) {
            // PROTOCOLO V205: Enviar comando de reset para aliviar processador
            val resetCmd = "cmd package compile --reset -a"
            // Executamos o reset imediatamente
            runDirectCommand(resetCmd)
            return@withContext "AVISO: Temperatura Crítica (${ThermalWatchdog.getTemperature(context)}°C). Sistema Resetado para resfriamento!"
        }
