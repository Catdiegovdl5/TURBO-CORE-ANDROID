...
@OptIn(ExperimentalMaterial3Api::class) class MainActivity : ComponentActivity() {
...
fun DesempenhoScreen() {
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Spacer(Modifier.height(16.dp)); ResetButton()
    }
}
@Composable
fun EconomiaScreen() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        OutlinedTextField(
            ...
            colors = OutlinedTextFieldDefaults.colors(
                focusedBorderColor = Color(0xFF00E5FF), unfocusedBorderColor = Color.Gray,
                focusedTextColor = Color.White, unfocusedTextColor = Color.White
            ),
            keyboardOptions = KeyboardOptions.Default.copy(imeAction = ImeAction.Go),
            keyboardActions = KeyboardActions(onGo = { scope.launch { output = runShizukuWithOutput(command) } })
        )
        CyberButton("EXECUTAR", Color(0xFF00E5FF)) { scope.launch { output = "Executando..."; output = runShizukuWithOutput(command) } }
        Card(modifier = Modifier.fillMaxWidth().height(300.dp), colors = CardDefaults.cardColors(containerColor = Color(0xFF171717)), border = androidx.compose.foundation.BorderStroke(1.dp, Color.Gray)) {
            Text(text = output, color = Color.Green, fontSize = 12.sp, fontFamily = androidx.compose.ui.text.font.FontFamily.Monospace, modifier = Modifier.padding(16.dp).verticalScroll(rememberScrollState()))
        }
    }
}
@Composable
fun ResetButton() {
    CyberButton("🔄 RESETAR TUDO", Color.Gray) {
        runShizukuCommand("wm size reset"); changeDpiSafely(null)
...
fun CyberButton(text: String, color: Color, onClick: () -> Unit) {
    Button(onClick = onClick, modifier = Modifier.fillMaxWidth().height(56.dp), colors = ButtonDefaults.buttonColors(containerColor = color.copy(alpha = 0.8f)), shape = MaterialTheme.shapes.small, border = androidx.compose.foundation.BorderStroke(1.dp, color)) {
        Text(text, fontSize = 16.sp, fontWeight = FontWeight.Bold, color = if(color == Color.White || color == Color.Green || color == Color(0xFF00E5FF)) Color.Black else Color.White)
    }
}
private inline fun safeRun(block: () -> Unit) { try { block() } catch (e: Exception) { Log.e("TurboCore", "Erro", e) } }
private fun changeDpiSafely(density: Int?) {
    val cmd = if (density == null) "wm density reset" else { if (density < 72 || density > 640) return; "wm density $density" }
